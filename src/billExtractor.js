import { getApiClient, mapApiError } from './apiClients.js';
...
- For paystubs: extract gross pay as one positive line, then each deduction (federal tax, state tax, FICA, 401k, Roth, insurance, etc.) as a positive line using the deduction name as description.
- For brokerage statements: extract dividends, interest, distributions as positive lines. SKIP buy/sell trades.
- For credit-card statements: each transaction is one item. Refunds / returns / credits → negative amount. Skip "PAYMENT - THANK YOU" lines (those are the user's payment to the card issuer, not transactions).
...
- Each item's "date" is the transaction's posting/purchase date in YYYY-MM-DD. If the printed date omits the year, infer it from the statement period (transactions in Dec on a Jan statement use the prior year).
...
export async function extractBillFromImage(imageDataUrl, { apiKey, model, provider = 'auto' }) {
  ...
  try {
    const result = getApiClient(apiKey, provider);
  ...
  let message;
  try {
    message = await apiClient.messagesCreate({
      model,
      max_tokens: 2048,
      messages: [
        {
    ...
    throw new Error(mapApiError(err, detectedProvider), { cause: err });
  }
